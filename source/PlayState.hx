...
class PlayState extends MusicBeatState
{
	...
	public static var DEFAULT_TIME_BG = FlxColor.GRAY;
	public static var DEFAULT_TIME_FILLED = FlxColor.LIME;
	public static var DEFAULT_HP_RED = 0xFFFF0000;
	public static var DEFAULT_HP_GREEN = 0xFF66FF33;
	...
	public static var songFilename:String = "";
	...
	override public function create()
	{
		...
		#if sys
		// executeModchart = Paths.doesLuaExistWeek(PlayState.SONG.song.toLowerCase() + "/modchart");//FileSystem.exists(Paths.luaWeek(PlayState.SONG.song.toLowerCase() + "/modchart"));
		executeModchart = FileSystem.exists(Paths.luaWeekPath(PlayState.SONG.song.toLowerCase() + "/modchart"));
		#end
		#if !cpp
		executeModchart = false; // FORCE disable for non cpp targets
		#end

		trace('Mod chart: ' + executeModchart + " - " + Paths.luaWeekPath(PlayState.SONG.song.toLowerCase() + "/modchart"));
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'senpai' | 'roses' | 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.txtWeek(PlayState.SONG.song.toLowerCase() + '/dialogue', "week" + storyWeek, "weeks"));
		}
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
		{
			...
			var songName = new FlxText(songPosBG.x + (songPosBG.width / 2) - 20, songPosBG.y, 0, visualSongName, 16);
			if (FlxG.save.data.downscroll)
				songName.y -= 3;
		...
		/*#if TESTING
		var testingWaterMark = new FlxText(4,healthBarBG.y + 50,0,"MOD TOOL TESTING BUILD", 16);
		testingWaterMark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		testingWaterMark.scrollFactor.set();
		testingWaterMark.updateHitbox();
		testingWaterMark.screenCenter(X);
		add(testingWaterMark);
		#end*/
	...
	private function generateSong(dataPath:String):Void // paramater is unused
	{
		...
		if (SONG.needsVoices)
			vocals.loadEmbedded(Paths.voicesWeek(PlayState.SONG.song, "week" + storyWeek));
		...
		#if desktop
			var songPath = Paths.weekPath(PlayState.SONG.song.toLowerCase() + '/', "week" + storyWeek);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			/*if(player == 1) {
				babyArrow.x = (Note.swagWidth * 3) - babyArrow.x;
		...
		if (generatedMusic)
			{
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if(!daNote.modifiedByLua) {
						...
						if(daNote.mustPress) {
							daNote.visible = playerStrums.members[noteData].visible;
							daNote.x = playerStrums.members[noteData].x;
							if (!daNote.isSustainNote)
								daNote.angle = playerStrums.members[noteData].angle;
							daNote.alpha = playerStrums.members[noteData].alpha;
						}
						else if(!daNote.wasGoodHit)
						{
							daNote.visible = strumLineNotes.members[noteData].visible;
							daNote.x = strumLineNotes.members[noteData].x;
							if (!daNote.isSustainNote)
								daNote.angle = strumLineNotes.members[noteData].angle;
							daNote.alpha = strumLineNotes.members[noteData].alpha;
						}
					}
	...
	function endSong():Void
	{
		...
		if (SONG.validScore)
		{
			Highscore.saveScore(currentMod, SONG.song, Math.round(songScore), storyDifficulty);
		}
		#end
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					trace(PlayState.storyPlaylist[0].toLowerCase() + difficulty + " - Visual Name: " + PlayState.storyVisNamePlaylist[0]);
	...
	*/
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && curSong.toLowerCase() == 'milf' && curBeat >= 168 && curBeat < 200)
		{
